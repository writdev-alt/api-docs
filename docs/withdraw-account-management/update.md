...
Update an existing withdrawal account. The bank account will be re-verified with the payment gateway in production environments.
...
|-----------|---------|----------|------------------------------------------|
| `id`      | integer | ✅       | The withdrawal account ID to update.     |
...
Same parameters as described in [Create Withdrawal Account](./create).
...
```bash
curl -X PUT "https://api.example.com/api/v1/withdraw-accounts/1" \
